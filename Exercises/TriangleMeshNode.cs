		...
		void CreateTrianglesVBO()
		{
			...
			for (int triangleIndex = 0; triangleIndex < mesh.GetNumberOfTriangles(); triangleIndex++)
			{
				...
				for (int vertexIndex = 0; vertexIndex < 3; vertexIndex++)
				{
					...
					if (t.GetTexCoordIndex(vertexIndex) >= 0)
					{
						texCoords = mesh.GetTextureCoordinate(t.GetTexCoordIndex(vertexIndex));
					}
					renderVertices.Add(new RenderVertex(pos, t.Normal, color, texCoords));
				}
			}
