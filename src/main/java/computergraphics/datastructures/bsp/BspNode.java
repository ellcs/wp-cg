...
public class BspNode extends LeafNode {
  ...
  private List<RenderVertex> CreateVBOElements(BspTreeNode node) {
    ...
    for (int or = 0; or < 2; or++) {
      BspTreeNode.Orientation orientation = BspTreeNode.Orientation.values()[or];
      Vector color = (orientation == BspTreeNode.Orientation.NEGATIVE) ? new Vector(0, 1, 1, 1) : new Vector(1, 0, 1, 1);
