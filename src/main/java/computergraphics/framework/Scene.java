...
//    final GLCapabilities tGLCapabilities = new GLCapabilities(glp);
  ...
  public void init(GL2 gl) {
    ...
    gl.glClearColor((float) root.getBackgroundColor().x(),
    ...
    ShaderAttributes.getInstance().getAttributes(gl,
        root.getShader().getProgram());
  }
  ...
  private void drawRegular(GL2 gl) {
    root.getShader().use(gl);
    ShaderAttributes.getInstance().setCameraEyeParameter(gl,
        root.getCamera().getEye());
    ShaderAttributes.getInstance().setShaderModeParameter(gl,
        root.getShader().getMode());
    ShaderAttributes.getInstance().setLightPositionParameter(gl,
        root.getLightPosition());
    Shader.checkGlError(gl);
    ...
    getRoot().traverse(gl, RenderMode.REGULAR, Matrix.createIdentity4());
  }
  ...
  private void drawShadowVolume(GL2 gl) {
    ...
    root.traverse(gl, RenderMode.REGULAR, Matrix.createIdentity4());
    ...
    // rendered).
    root.traverse(gl, RenderMode.SHADOW_VOLUME, Matrix.createIdentity4());
  ...
  private void drawDebugShadowVolume(GL2 gl) {
    ...
    ShaderAttributes.getInstance().setShaderModeParameter(gl,
        root.getShader().getMode());
    getRoot().traverse(gl, RenderMode.DEBUG_SHADOW_VOLUME,
        Matrix.createIdentity4());
  ...
  private void updateProjectionMatrix(GL2 gl) {
    if (needsUpdateProjectionMatrix) {
      ...
      gl.glLoadIdentity();
      glu.gluPerspective(root.getCamera().getFovy(),
          root.getCamera().getAspectRatio(), root.getCamera().getZNear(),
  ...
  private void updateViewMatrix(GL2 gl) {
    if (needsUpdateViewMatrix) {
      ...
      glu.gluLookAt(cam.getEye().x(), cam.getEye().y(), cam.getEye().z(),
          cam.getRef().x(), cam.getRef().y(), cam.getRef().z(), cam.getUp().x(),
          cam.getUp().y(), cam.getUp().z());
  ...
  public void rotateCamera(double angleAroundUp, double angleUpDown) {
    root.getCamera().updateExtrinsicCameraParams((float) angleAroundUp,
        (float) angleUpDown);
  ...
  public void keyPressedEvent(char keyChar) {
    int key = Character.toUpperCase(keyChar);
    switch (key) {
      case 'I':
